			...
			<main class={style.main}>
				<section class={style.intro}>
					<div class={style.profileSection}>
						<img src="/assets/Diana6Gflagship.jpg" alt="Diana Moya" class={style.profilePicture} />
						<div class={style.profileButtons}>
							<a href="https://www.google.com/url?q=https%3A%2F%2Fbr.linkedin.com%2Fin%2Fdiana-pamela-moya-osorio-508a9b172&sa=D&sntz=1&usg=AOvVaw0KFpAfx4GYVNrYhr4Y0VYo" class={style.roundButton} aria-label="LinkedIn">
								<img src="/assets/LI-In-Bug.png?height=24&width=24" alt="LinkedIn" class={style.buttonIcon} />
							</a>
							<a href="https://scholar.google.ch/citations?hl=de&user=RgcMUu4AAAAJ&view_op=list_works&sortby=pubdate" class={style.roundButton} aria-label="Google Scholar">
								<img src="/assets/Scholar_logo.svg?height=24&width=24" alt="Google Scholar" class={style.buttonIcon} />
							</a>
							<a href="https://elliit.se/wp-content/uploads/2023/11/ELLIIT_Recruited_Faculty_catalogue_Diana_MoyaOsorio.pdf" class={style.roundButton} aria-label="Elliit">
								<img src="/assets/Untitled.png?height=24&width=24" alt="Elliit" class={style.buttonIcon} />
							</a>
						</div>
					</div>
...
const ResearchHighlight = ({ title, description, imageUrl, link }) => {
	return (
		<a href={link} class={style.highlight} target="_blank" rel="noopener noreferrer">
			<img src={imageUrl} alt={title} class={style.highlightImage} />
			<h3>{title}</h3>
			<p>{description}</p>
		</a>
	);
